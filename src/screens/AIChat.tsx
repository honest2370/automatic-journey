...
export default function AIChat() {
  ...
  const processSend = async (text: string, sid: string) => {
    ...
    const userMsg: AiMessage = {
      id: uid, session_id: sid, role: "user",
      content, created_at: new Date().toISOString(),
    };
    setMessages((prev) => [...prev, userMsg]);
    await supabase.from("ai_messages").insert({ session_id: sid, role: "user", content });
    ...
    <div className="relative flex h-dvh flex-col" style={{ background: "#f8fafc" }}>
            ...
            <div className="flex-1 overflow-y-auto px-2 py-2">
              {sessions.length === 0 ? (
                ...
                >
                  ...
                  <button
                    onClick={() => del(s.id)}
                    className="hidden group-hover:flex p-1.5 text-rose-400 hover:bg-rose-50 rounded-lg"
                  >
      ...
      <header className="safe-top shrink-0 z-30 bg-gradient-to-r from-[#060c20] to-[#0a3d6e]">
        <div className="flex items-center gap-2 px-3 py-2.5">
          <button
            onClick={back}
            className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/10 border border-white/10"
          >
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto no-scrollbar">
        {!sessionId ? (
          /* ── WELCOME SCREEN ── */
          <div className="flex min-h-full flex-col items-center px-5 py-10 animate-fade">
            {/* brand hero */}
            <div className="mb-6 flex flex-col items-center">
              <div className="relative mb-4">
                <div className="absolute inset-0 rounded-3xl bg-cyan-400/30 blur-2xl scale-110" />
                <div className="relative flex h-20 w-20 items-center justify-center rounded-3xl gradient-brand shadow-xl shadow-cyan-500/30">
                  <LogoMark size={46} />
                </div>
              </div>
              <h2 className="text-[22px] font-extrabold text-base text-center">Bonjour, je suis ADF IA</h2>
              <p className="mt-1.5 max-w-xs text-center text-[13px] text-muted leading-relaxed">
                Votre assistant intelligent pour découvrir nos services, commander et explorer la boutique ADF.
              </p>
            ...
            <div className="w-full max-w-sm space-y-2 mb-6">
              ...
              {AI_SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => startChat(s)}
                  className="flex items-center gap-3 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-left transition hover:border-accent-300 hover:bg-accent-50/50 active:scale-[.98]"
                >
          ...
          <div className="flex items-center justify-between text-[10px] text-muted/60">
            ...
            {source && (
              <span className="flex items-center gap-1">
                ...
                {source === "online" ? "En ligne" : "Hors-ligne"}
              </span>
            )}
          </div>
      ...
      {/* ── COMPOSER ── */}
      <div className="safe-bottom shrink-0 border-t border-slate-100 bg-white/95 backdrop-blur px-3 pb-2.5 pt-2.5">
