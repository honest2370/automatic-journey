...
import {
  HomeIcon, SparkIcon, BagIcon, ReceiptIcon, BotIcon,
} from "./lib/icons";
...
function Shell() {
  ...
  // ——— Routing guard: runs only once profile is loaded ———
  useEffect(() => {
    if (!session || !profile || !ready) return;
    if (route.name === "home") return;

    // Admin route: allow admins, bounce non-admins to home
    if (route.name === "admin") {
      if (profile.role !== "admin") replace({ name: "home" });
      return;
    }

    // All other valid deep-links: keep them
    const keepRoutes = ["settings", "services", "store", "orders", "ai", "order-detail"];
    if (!keepRoutes.includes(route.name)) {
      replace({ name: "home" });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [session?.user?.id, profile?.id, profile?.role, ready]);
  ...
  switch (route.name) {
    ...
    case "admin":
      // Profile may still be loading — show spinner until we know the role
      if (!profile) {
        screen = <div className="flex h-dvh items-center justify-center"><div className="h-8 w-8 rounded-full border-4 border-accent-200 border-t-accent-500 animate-spin" /></div>;
      } else {
